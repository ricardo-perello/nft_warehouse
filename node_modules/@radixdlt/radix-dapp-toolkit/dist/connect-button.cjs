...
var U = class extends import_lit.LitElement {
  constructor() {
...
var j = class extends import_lit.LitElement {
  constructor() {
...
var C = class extends import_lit.LitElement {
  constructor() {
...
C.styles = [
  ...
  import_lit.css`
      ...
      button.logo::before {
        content: '';
        mask-image: url(${(0, import_lit.unsafeCSS)(tt)});
        -webkit-mask-image: url(${(0, import_lit.unsafeCSS)(tt)});
      }
      ...
      button.gradient {
        ...
        background-image: url(${(0, import_lit.unsafeCSS)(St)});
        padding-right: 7px;
      }
      ...
      :host([status='default']) > button.gradient::before {
        mask-image: url(${(0, import_lit.unsafeCSS)(et)});
        -webkit-mask-image: url(${(0, import_lit.unsafeCSS)(et)});
        width: 22px;
        min-width: 22px;
        height: 22px;
      ...
      :host([status='success']) > button::before {
        mask-image: url(${(0, import_lit.unsafeCSS)(it)});
        -webkit-mask-image: url(${(0, import_lit.unsafeCSS)(it)});
        width: 22px;
        min-width: 22px;
        height: 22px;
        -webkit-mask-position: center;
      ...
      :host([status='error']) > button::before {
        mask-image: url(${(0, import_lit.unsafeCSS)(ot)});
        -webkit-mask-image: url(${(0, import_lit.unsafeCSS)(ot)});
        width: 22px;
        min-width: 22px;
        height: 22px;
        -webkit-mask-position: center;
      ...
      @container (width < ${G - 0.1}px) {
        ...
        button.gradient {
          background-image: url(${(0, import_lit.unsafeCSS)(Et)});
          padding: 0;
        }
        button.logo::before {
          font-size: 16px;
...
var v = class extends import_lit.LitElement {
  constructor() {
    super(...arguments), this.header = "";
  }
  render() {
    const t = () => this.timestamp ? import_lit.html`<div class="timestamp">${ut(this.timestamp)}</div>` : "", i = `${this.icon ? "30px" : ""} 1fr ${this.timestamp ? "42px" : ""}`;
...
var Y = class extends import_lit.LitElement {
  constructor() {
...
var A = class extends import_lit.LitElement {
  constructor() {
  ...
  render() {
    ...
    }}">${Nt(this.address)} <button aria-label="Copied!" class="${(0, import_class_map.classMap)({
...
var x = class extends import_lit.LitElement {
  constructor() {
  ...
  render() {
    const t = this.getIconFromStatus(), i = this.getStylingFromStatus(), r = {
      sendTransaction: {
        ...
        content: import_lit.html`${this.renderTxIntentHash()} ${this.status === "pending" ? import_lit.html`<div class="request-content">Open your Radix Wallet app to review the transaction ${this.showCancel ? import_lit.html`<div class="cancel" @click="${this.onCancel}">Cancel</div>` : import_lit.html`<div class="cancel" @click="${this.onIgnore}">Ignore</div>`}</div>` : ""}`
      },
      dataRequest: {
  ...
  getRequestContentTemplate(t) {
    return this.status === m.pending ? import_lit.html`<div class="request-content">${t} ${this.showCancel ? import_lit.html`<div class="cancel" @click="${this.onCancel}">Cancel</div>` : ""}</div>` : "";
  }
  isErrorStatus(t) {
    return [
      m.cancelled,
  ...
  renderTxIntentHash() {
    return this.transactionIntentHash ? import_lit.html`<div class="transaction"><span class="text-dimmed">ID:</span><radix-link displayText="${Nt(this.transactionIntentHash)}" @click="${(t) => {
      t.preventDefault(), this.dispatchEvent(
        new CustomEvent("onLinkClick", {
          bubbles: true,
          composed: true,
...
var W = class extends import_lit.LitElement {
  ...
  render() {
    return (this.requestItems || []).map(
      (t) => import_lit.html`<radix-request-card type="${t.type}" status="${t.status}" id="${t.interactionId}" ?showCancel="${t.showCancel}" timestamp="${t.createdAt}"></radix-request-card>`
    );
  }
};
W.styles = [H];
...
var F = class extends import_lit.LitElement {
  render() {
...
var N = class extends import_lit.LitElement {
  constructor() {
...
var b = class extends import_lit.LitElement {
  constructor() {
  ...
  render() {
    return import_lit.html`<div class="header">Sharing with ${this.dAppName}</div><div class="content"><radix-persona-card avatarUrl="${this.avatarUrl}" persona="${this.persona}" .personaData="${this.personaData}"></radix-persona-card><div>${(this.accounts || []).map(
      ({ label: t, address: i, appearanceId: r }) => import_lit.html`<radix-account label="${t}" address="${i}" appearanceId="${r}"></radix-account>`
...
var T = class extends import_lit.LitElement {
  constructor() {
    super(...arguments), this.requestItems = [], this.dAppName = "", this.loggedInTimestamp = "";
  }
  render() {
    return import_lit.html`<div class="header">Connected to ${this.dAppName}</div><slot name="subheader"></slot>${this.loggedInTimestamp ? import_lit.html`<div class="subheader">Since logged in: ${ut(this.loggedInTimestamp, ", ")}</div>` : ""}<div class="content">${(this.requestItems || []).map(
      (t) => import_lit.html`<radix-request-card type="${t.type}" status="${t.status}" id="${t.interactionId}" transactionIntentHash="${t.transactionIntentHash || ""}" ?showCancel="${t.showCancel}" timestamp="${t.createdAt}"></radix-request-card>`
    )}</div>`;
  }
};
T.styles = [
  Dt,
  import_lit.css`.subheader{color:var(--radix-card-text-dimmed-color);margin-top:-12px;margin-bottom:15px;text-align:center;font-size:12px}.content{padding-bottom:25px;max-height:calc(100vh - 270px)}@media (min-height:580px){.content{max-height:360px}}`
];
X([
  (0, import_decorators.property)({ type: Array })
...
var l = class extends import_lit.LitElement {
  constructor() {
...
var Q = class extends import_lit.LitElement {
  constructor() {
    super(...arguments), this.header = "Radix Wallet Connector";
  }
  render() {
    return import_lit.html`<div class="radix-wallet-connector-card"><div class="radix-wallet-connector-card__logo"><img width="78" height="78" src="${At}" alt="Radix Wallet Connector Logo"></div><div class="radix-wallet-connector-card__header">${this.header}</div><slot></slot></div>`;
  }
};
Q.styles = [
...
var L = class extends import_lit.LitElement {
  ...
  render() {
    return import_lit.html`<div class="connector-error">${this.isError ? import_lit.html`<img src="${ue}" alt="Warning" width="33">` : ""}<h1 class="${(0, import_class_map.classMap)({ error: this.isError })}">${this.header}</h1>${this.subheader ? import_lit.html`<h3>${this.subheader}</h3>` : ""}</div>`;
  }
};
L.styles = [
...
var D = class extends import_lit.LitElement {
  constructor() {
